...
var Milkbox = new Class({
	...
	openMilkbox:function(gallery,index){
		...
		this.overlay.setStyles({ 'top': -document.id(window).getScroll().y,'height':document.id(window).getScrollSize().y+document.id(window).getScroll().y });
	...
	setGalleriesEvents:function(){
		this.galleries.each(function(gallery){
			...
			$$(gallery).addEvent('click',function(e){
				var button=(document.id(e.target).match('a')) ? document.id(e.target) : document.id(e.target).getParent('a');
				e.preventDefault();
	...
	prepareGalleries:function(responseElements){
		...
		milkbox_a.each(function(a){
			document.id(a).store('href',a.href);
			document.id(a).store('rel',a.rel);
			document.id(a).store('title',a.title);
			if(this.checkFileType(a.href,"swf")){ this.setFileProps(document.id(a),a.rev); }

			if(this.options.removeTitle){ document.id(a).removeProperty('title'); }
			if(a.rel.length > 7){
				this.families.each(function(f,i){
					if(a.rel == f){
						...
						if(gMounted == true){ this.galleries[index].push(document.id(a)); } 
						else { this.galleries.push([document.id(a)]); }
					};
				},this);
			} else { this.galleries.push([document.id(a)]); };
		},this);
	...
	prepareHTML:function(){		
		...
		this.overlay = new Element('div', { 'id':'mbOverlay','styles':{ 'opacity':0,'visibility':'visible','height':0,'overflow':'hidden' }}).inject(document.id(document.body));
		
		this.center = new Element('div', {'id':'mbCenter', 'styles':{'width':this.options.initialWidth,'height':this.options.initialHeight,'marginLeft':-(this.options.initialWidth/2),'opacity':0 }}).inject(document.id(document.body));
